...
func _is_valid_placement(world_pos: Vector2) -> bool:
	...
	var atlas := tilemap.get_cell_atlas_coords(tile_pos)
	if atlas == WATER_TILE:
		return false
	for machine in get_tree().get_nodes_in_group("machines"):
...
func _place_machine(world_pos: Vector2) -> void:
	...
	match _placing_machine:
		...
		"Anvil":
			machine = ANVIL_SCRIPT.new()
		_:
			# Extrusion Machine or unknown — place a placeholder
			machine = Node2D.new()
			var lbl := Label.new()
...
func _generate_pois() -> void:
	var poi_types: Array[Dictionary] = [
		{name="Iron Deposit",      color=Color(0.55, 0.55, 0.58),  count=3, cluster=6},
		{name="Copper Vein",       color=Color(0.80, 0.45, 0.20),  count=3, cluster=5},
		{name="Coal Seam",         color=Color(0.18, 0.18, 0.20),  count=3, cluster=8},
		{name="Crystal Formation", color=Color(0.30, 0.80, 0.90),  count=2, cluster=4},
		{name="Geothermal Vent",   color=Color(0.90, 0.35, 0.10),  count=2, cluster=3},
		{name="Gold Seam",         color=Color(0.90, 0.78, 0.10),  count=2, cluster=3},
	]
...
func _build_grass_layer() -> void:
	...
	# Each SBS tile (256×128) covers 8×8 logical tiles (32×16).
	# Extend one extra tile beyond the logical map edge for full coverage.
	const SBS_HALF: int = 8
...
func _place_dirt_patches() -> void:
	# Scatter dirt clusters so dirt is a minority (~20% coverage)
				...
				for dx in range(-r, r + 1):
					for dy in range(-r, r + 1):
						if dx * dx + dy * dy <= r * r:
							...
							if abs(cell.x) <= HALF_SIZE and abs(cell.y) <= HALF_SIZE:
								tilemap.set_cell(cell, _source_id, DIRT_TILE)

func _place_water_pools() -> void:
	var num_pools := rng.randi_range(6, 14)
	for _i in num_pools:
	...
	for x in range(-HALF_SIZE + 2, HALF_SIZE - 1):
		for y in range(-HALF_SIZE + 2, HALF_SIZE - 1):
			if abs(x) < 8 and abs(y) < 8:
				continue
			if tilemap.get_cell_atlas_coords(Vector2i(x, y)) == WATER_TILE:
				continue
			if _near_vent(x, y):
				continue
			if rng.randf() < 0.15:
